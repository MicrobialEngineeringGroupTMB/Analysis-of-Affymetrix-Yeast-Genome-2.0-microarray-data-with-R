...
### 1. Introduction
The R notebook stored in this repository contains the code used to process Affymetrix Yeast Genome 2.0 microarray data for the Almeida et al (2022) publication titled *Physiological and molecular characterization of yeast cultures pre-adapted for fermentation of lignocellulosic hydrolysate*. The purpose of this repository is three-fold: to archive the code, to contribute to reproducible science, and for the sake of education. 
...
```{r,}
files = list.files("C:/GSE218764/raw_data", full.names = TRUE)
...
Perform RMA normalization on the raw data we've loaded and convert the normalized data to an expression set for downstream use:
```{r, message = FALSE, warning=FALSE}
...
colnames(expr_set_rma) = c("CONT.1", "CONT.2", "HYD9h.1", "HYD9h.2", 
                             "HYD17h.1", "HYD17h.2", "SUG9h.1", "SUG9h.2",
                             "SUG17h.1","SUG17h.2")
```
...
```{r,}
write.table(expr_set_rma, file = "C:/GSE218764/expr_set_rma.tsv", quote = FALSE, sep = "\t", col.names=NA)
```
...
Inspect the top five rows in the PCA we made for the samples. Principal components are sorted by how much of the variance they describe, and we will be using PC1 and PC2 for the cluster plot.
```{r,}
pca_all_summary <- summary(PCA_all_s)
pca_all_summary$importance[, 1:5]
```

To plot the PCA results with ggplot, we first convert the results to a dataframe, label the samples in a column called group, and then use the dataframe as input for ggplot. Here we set the x- and y-axes to the same intervals (-100,100), since square PCA plots can sometimes be easier to interpret:

```{r,}
pca_all_df <- data.frame(PCA_all_s$x[, 1:2])
pca_all_df$group <- c('Control 0h','Control 0h','Hydrolysate 9h','Hydrolysate 9h',
...
The PCA plot indeed shows that the replicates (.1 and .2) of each samples generally cluster together, and it could possibly be said that the the 9h samples form a larger cluster. This shows that the experimental setup overall seem to be reproducible. We will repeat the PCA later with only the *S. cerevisiae* probes and see if that affects the clustering. 
...
```{r,}
ggsave("C:/GSE218764/pca_rma_all.pdf", width = 9, height = 6, units = 'in')
ggsave("C:/GSE218764/pca_rma_all.tiff", width = 9, height = 6, units = 'in',compression = "lzw")
```
...
Now that we have the normalized data, we need to specify which sample belongs to which group, setup a so called *design matrix*, fit the data to the design matrix, define which groups we want to compare in a *contrast matrix* and finally perform the contrasts and thus the differential expression analysis.

Depending on how well the chips where annotated when they were analysed in the lab, the .CEL files might contain useful annotations, which can be stored in the phenodata object in the array data loaded by ReadAffy.
#read the phenodata object from the array indata and classify each sample by their group. 

However, with the current data, there were no annotations in phenoData:
```{r,}
ph = affy_data@phenoData
ph@data
```

Since this data contains no information on which group each sample belongs to, we need to add it ourselves. We can write it to the phenoData dataframe (ph) if want, or just to a new vector. Let's add it to the dataframe in this example. Since the .CEL file names start with an integer, we know which order they were loaded. And we can thus create a column vector containing the groups, in the same order:
```{r,}
ph@data[ ,2] = c("control","control","HYD9h","HYD9h", "HYD17h","HYD17h","SUG9h","SUG9h","SUG17h","SUG17h")
...
```{r,}
f = factor(ph@data$groups,levels = c("control","HYD9h","HYD17h","SUG9h","SUG17h"))
design = model.matrix(~ 0 + f) 
colnames(design)=c("control","HYD9h","HYD17h","SUG9h","SUG17h")
design #display the final matrix
```
...
Make a contrast matrix to decide which conditions to compare. Here we want to contrast the 9h samples by comparing HY9h to SUG9h. The order of the contrast decides the direction of the fold change. By calling the contrast in the order HYD9h-SUG9h, positive fold changes will mean that a probe had higher expression levels in the Hyd9h samples, and negative that the probe had lower expression levels in the Hyd9h samples. If we call SUG9h-HYD9h we will get the opposite direction, which is not what we want for the current analysis. 

```{r,}
contrast_matrix = makeContrasts(HYD9h-SUG9h,HYD17h-SUG17h,levels=design)
```
...
Later on we will be using Limma's *decideTests* function to statistically test for differentially expressed (DE) genes below a specified False Discovery Rate (FDR) p-value threshold.  The standard work-flow in Limma is to calculate p-values using the *eBayes* function and then use *decideTests* to implement FDR correction and test for significant genes. However, it is common in transcriptomics analysis to further narrow down the list of significant genes using a log2 fold change cut-off in addition to the p-value cut-off (which is really the important cut-off). But although *decideTests* can perform the test on both FC and p-value cut-off, the authors recommend against this since it is a form of post-hoc filtering. Instead, they authors suggest to use the *treat* function instead of *ebayes* 

See the following note from the Limma manual for *decideTests()*:
...
```{r,}
data_fit_tr = treat(data_fit_con, lfc=log2(2.0))
```
...
For this analysis we decided that we wanted to use the same t-value cut-off for both our contrasts (Hyd9 vs Sug9h, and Hyd17 vs Sug17h) and thus we decided to use the "global" method in *decideTests*. For FDR correction we can specify that we want to use Benjamini–Hochberg (BH) adjusted p-values:
...
To get the log2 fold changes (log2FC) for all probes we can use use *topTreat()* with "number=Inf". This will also gives us adjusted p-values, however, they will be generated from the "separate" method (!) and not the "global" method. We will work on addressing this below, and will basically drop the p-values from topTreat and replace them the the "global" method ones.

```{r,}
tab_tr_HYD9h_vs_SUG9h = topTreat(data_fit_tr,coef=1,number=Inf,adjust.method='BH')
tab_tr_HYD17h_vs_SUG17h = topTreat(data_fit_tr,coef=2,number=Inf,adjust.method='BH')
```
...
#note that data_fit_tr is the same fit object that was give to decideTests().
```
...
adj_p_glob now contains all the Benjamini–Hochberg adjusted p values for both contrasts in a column vector, so to split them by contrast(Hyd9 vs Sug9h, and Hyd17 vs Sug17h) we can use the fact that there are 10928 unique probes in these .CEL files, and just use the first and second half of the vector to populate columns in our dataframe:
```{r,}
df_dt_glob$adj.P.Val_9h <- adj_p_glob[1:10928] 
df_dt_glob$adj.P.Val_17h <- adj_p_glob[10929:21856] 
```

If we want to save the data to file, we can use the *write.fit()* function from Limma:
```{r,}
...
So far we have the DE data for the probes, but to make any biologically relevant interpretations from the data we first need to annotate them with gene information.
...
To call all probes in the package and fetch the ORF name, gene name and *S. cerevisia database* ID (an unique identifier for each ORF that will remain the same even if the ORF and/or gene name changes over time in different database updates), specify the data as column and the probves as keytype:
```{r,}
...
Basically. if the command returns the message "'select()' returned 1:1 mapping between keys and columns" then a single row has been added per probe

We can inspect the data we fetched for instance by:
```{r,}
...
It is possible to get chromosome coordinates from this package, as described in the [manual](https://bioconductor.org/packages/release/data/annotation/html/yeast2.db.html). However, in this example we will instead use Ensembl to pull all the chromosome coordinate-related data in a single query. 

##### 5.2. Fetching annotations from Ensembl

An alternative to the yeast2.db package is to programmatically access Ensembl and pull all the data for *S.cerevisiae* we need. It seems that yeast2.db is slightly more accurate e.g. for probes that anneal to hypothetical ORFs that overlap each other, and therefore we will use that as our main source of annotation. Just keep in mind that we could either of these three options on their own, but here we combine them to benefit from their different strenghts. 

To connect to the Ensembl database and fetch *S. cerevisiae* dataset:
```{r,}
...
ensemble_annotation <- getBM(attributes=c('affy_yeast_2','ensembl_gene_id','external_gene_name', 'sgd_gene',
...
```{r,}
tab_tr_final <- merge(probe_annotation_rownames, 
                      expr_set_rma,
...
```{r,}
tab_tr_HYD9h_vs_SUG9h_temp <- tab_tr_HYD9h_vs_SUG9h
colnames(tab_tr_HYD9h_vs_SUG9h_temp)[colnames(tab_tr_HYD9h_vs_SUG9h_temp) == 'logFC'] <- 'hyd9h_vs_sug9h log2FC'
colnames(tab_tr_HYD9h_vs_SUG9h_temp)[colnames(tab_tr_HYD9h_vs_SUG9h_temp) == 't'] <- 'hyd9h_vs_sug9h t'
colnames(tab_tr_HYD9h_vs_SUG9h_temp)[colnames(tab_tr_HYD9h_vs_SUG9h_temp) == 'P.Value'] <- 'hyd9h_vs_sug9h P.value'

tab_tr_HYD17h_vs_SUG17h_temp <- tab_tr_HYD17h_vs_SUG17h
colnames(tab_tr_HYD17h_vs_SUG17h_temp)[colnames(tab_tr_HYD17h_vs_SUG17h_temp) == 'logFC'] <- 'hyd17h_vs_sug17h log2FC'
colnames(tab_tr_HYD17h_vs_SUG17h_temp)[colnames(tab_tr_HYD17h_vs_SUG17h_temp) == 't'] <- 'hyd17h_vs_sug17h t'
colnames(tab_tr_HYD17h_vs_SUG17h_temp)[colnames(tab_tr_HYD17h_vs_SUG17h_temp) == 'P.Value'] <- 'hyd17h_vs_sug17h P.value'
```

Drop the adjusted p-values from the *topTreat()* function, since they are generated by the "separate" method:
```{r,}
tab_tr_HYD9h_vs_SUG9h_temp <- subset(tab_tr_HYD9h_vs_SUG9h_temp, select = -c(adj.P.Val))
tab_tr_HYD17h_vs_SUG17h_temp <- subset(tab_tr_HYD17h_vs_SUG17h_temp, select = -c(adj.P.Val))
```

The column *AveExpr* exists in both of the contrast dataframes and is identical, so we can drop it from the 17h results by:
```{r,}
tab_tr_HYD17h_vs_SUG17h_temp <- subset(tab_tr_HYD17h_vs_SUG17h_temp, select = -c(AveExpr))
```

To allow for a new round of *merge()* we need to convert the *Row.names* column generated by the last merge, back to row names:
```{r,}
tab_tr_final <- tab_tr_final %>% remove_rownames %>% column_to_rownames(var="Row.names") 
```

Now we can join it with the 9h results: 
```{r,}
tab_tr_final <- merge(tab_tr_final, 
                      tab_tr_HYD9h_vs_SUG9h_temp,
                      by = 'row.names', all = TRUE)
```
...
```{r,}
tab_tr_final <- tab_tr_final %>% relocate(AveExpr, .after = SUG17h.2)
```
...
```{r,}
tab_tr_final <- tab_tr_final %>% relocate('hyd9h_vs_sug9h adj.P.value (global)', .after = 'hyd9h_vs_sug9h P.value')
tab_tr_final <- tab_tr_final %>% relocate('hyd9h_vs_sug9h decideTests(global) results', .after = 'hyd9h_vs_sug9h adj.P.value (global)')
tab_tr_final <- tab_tr_final %>% relocate('hyd17h_vs_sug17h adj.P.value (global)', .after = 'hyd17h_vs_sug17h P.value')
```

Rename the columns with the normalized data to specify that they actually are normalized:
```{r,}
colnames(tab_tr_final)[colnames(tab_tr_final) == 'CONT.1'] <- 'CONT.1 (RMA norm.)'
colnames(tab_tr_final)[colnames(tab_tr_final) == 'CONT.2'] <- 'CONT.2 (RMA norm.)'
colnames(tab_tr_final)[colnames(tab_tr_final) == 'HYD9h.1'] <- 'HYD9h.1 (RMA norm.)'
colnames(tab_tr_final)[colnames(tab_tr_final) == 'HYD9h.2'] <- 'HYD9h.2 (RMA norm.)'
colnames(tab_tr_final)[colnames(tab_tr_final) == 'HYD17h.1'] <- 'HYD17h.1 (RMA norm.)'
colnames(tab_tr_final)[colnames(tab_tr_final) == 'HYD17h.2'] <- 'HYD17h.2 (RMA norm.)'
colnames(tab_tr_final)[colnames(tab_tr_final) == 'SUG9h.1'] <- 'SUG9h.1 (RMA norm.)'
colnames(tab_tr_final)[colnames(tab_tr_final) == 'SUG9h.2'] <- 'SUG9h.2 (RMA norm.)'
colnames(tab_tr_final)[colnames(tab_tr_final) == 'SUG17h.1'] <- 'SUG17h.1 (RMA norm.)'
colnames(tab_tr_final)[colnames(tab_tr_final) == 'SUG17h.2'] <- 'SUG17h.2 (RMA norm.)'
# colnames(tab_tr_final)[colnames(tab_tr_final) == 'ensembl_gene_id'] <- 'ORF'
# colnames(tab_tr_final)[colnames(tab_tr_final) == 'external_gene_name'] <- 'GENENAME'
colnames(tab_tr_final)[colnames(tab_tr_final) == 'Row.names'] <- 'Probe'
```

Write the final results to file:
```{r,}
write.table(tab_tr_final, file = "C:/GSE218764/tab_tr_final.tsv", quote = FALSE, sep = "\t", row.names = FALSE)
```
...
```{r,}
tab_tr_final_sc <- subset(tab_tr_final, Organism=="Saccharomyces cerevisiae") 
```
...
```{r,}
scer_probes <- tab_tr_final_sc[,"Probe"]
...
```{r,}
dt_glob_sc <- dt_glob[row.names(dt_glob) %in% tab_tr_final_sc[,1], ]
summary(dt_glob_sc)
```
...
But to be honest, graphically we could do with something prettier than the standard R Venn diagram. So instead, lets use a Venn diagram package that is based on ggplot: ggVennDiagram. The process is a little different this time, since we need to give the subfunction *Venn()* a list of the DE ORFs from each time point. We can extract them from the final dataset based on the outcome of *decideTests()* by taking all probes that scored a 1 or -1 in the test column: 

```{r,}
dt_glob_list_9h_orfs <- tab_tr_final_sc[tab_tr_final_sc$'hyd9h_vs_sug9h decideTests(global) results' != 0,'ORF']
dt_glob_list_17h_orfs <- tab_tr_final_sc[tab_tr_final_sc$'hyd17h_vs_sug17h decideTests(global) results' != 0,'ORF']
```
...
```{r,}
ggsave("C:/Users/Daniel/Dropbox/Work-from-home_folder/Joao microarray/venn.pdf", width = 6, height = 5, units = 'in')
ggsave("C:/Users/Daniel/Dropbox/Work-from-home_folder/Joao microarray/venn.tiff", width = 6, height = 5, units = 'in')

```

### 8. Volcano plots using the *S. cerevisiae* probe results

As a last part of this microarray analysis work flow we will plot our data in the form of volcano plots, which is a common way of presenting transcriptome data. In this case we have a small number of DE genes to begin with, so we can try to add labels to the volcano plots if there is room for them.

We will probably be limited in space in the volcano plots, so we will prioritize the probes with the smallest adjusted p.value. Extract the probes identified by *decideTests()* and sort them by adjusted p-value:
```{r,}
volcano_gene_labels_9h_glob <-tab_tr_final[tab_tr_final$"hyd9h_vs_sug9h decideTests(global) results" !=0, ]
...
volcano_gene_labels_17h_glob <-tab_tr_final[tab_tr_final$"hyd17h_vs_sug17h decideTests(global) results" !=0, ]
...
```{r,message = FALSE, warning=FALSE}
v1_tr_glob <- EnhancedVolcano(tab_tr_final_sc,
                             #lab=NA,      
                             lab = tab_tr_final_sc$GENENAME,
...
```{r, message = FALSE, warning=FALSE}
v2_tr_glob <- EnhancedVolcano(tab_tr_final_sc,
                             #lab=NA,      
                             lab = tab_tr_final_sc$GENENAME,
